...
func TestStoreListAllIncludesEverything(t *testing.T) {
	s, _ := newStore(t)
	mustInsertShare(t, s, sampleShare("a", "public", futureTS(1*time.Hour)))
	mustInsertShare(t, s, sampleShare("b", "private", pastTS(1*time.Hour)))
	mustInsertShare(t, s, sampleShare("c", "public", futureTS(1*time.Hour)))
	list := s.ListAll()
	if len(list) != 3 {
...
func TestStoreDeleteRemovesRow(t *testing.T) {
	s, _ := newStore(t)
	mustInsertShare(t, s, sampleShare("del", "public", futureTS(1*time.Hour)))
