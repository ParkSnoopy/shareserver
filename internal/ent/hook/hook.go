...
// The LoginFailureEventFunc type is an adapter to allow the use of ordinary
// function as LoginFailureEvent mutator.
type LoginFailureEventFunc func(context.Context, *ent.LoginFailureEventMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f LoginFailureEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.LoginFailureEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LoginFailureEventMutation", m)
}
